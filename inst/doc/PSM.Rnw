...
\emph{R}, and estimation times are thus in no way comparrable
...
\begin{eqnarray}
  ...
  \bs y_{ij} &=& \bs h(\bs x_{ij},\bs u_{ij},t_{ij},\bs\phi_i) + \bs e_{ij}
\end{eqnarray}
...
In the evaluation of the non-linear model is necessary to specify a
Jacobian matrix function with first-order partial derivatives for $\bs
f$ and $\bs h$. These functions are defined as

\begin{equation}
\frac{\partial \bs f}{\partial \bs x_t} ~~, ~~ \frac{\partial \bs h}{\partial \bs x_t}
\end{equation}

\noindent and must be given with the model specification. 
...
Kalman Filter for linear models which gives an exact solution. For
...
The PSM program is accessed in R through three main functions:
...
For more information please also refer to the help files in R.
...
\noindent where $\bs A_t = [ A_1 ~ A_2]^T$ are the amount in each compartment and thus
$A_1/V_1^i$ is the measured concentration. It is seems that the
...
The simulated data are shown in Figure \ref{fig:fig1}. The first row
shows the observations for individuals 1 and 2 and the next two show
state 1 and 2 which we wish to estimate.

\begin{figure}[htb]
\begin{center}
%<<label=figure1,fig=TRUE,echo=FALSE,height=3.5,width=5>>=
...
The first two states of the simulation model is concentraion in
